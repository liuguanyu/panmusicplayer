        ...
        <div class="flex-1 flex flex-col justify-between">
          <div>
            <div class="flex items-center gap-2">
              <h1 class="text-2xl font-bold">{{ playlist?.name || '加载中...' }}</h1>
...
const fetchPlaylistDetail = async () => {
  ...
  loading.value = true;
  try {
    const result = await playlistStore.getPlaylistById(playlistId.value);
    if (!result) {
...
const handlePlayAll = () => {
  ...
  playerStore.setPlaylist(playlist.value.tracks, true);
